...
public class DateTypeAdapter implements JsonDeserializer<Date>, JsonSerializer<Date> {
	protected final DateFormat enUsFormat;
	protected final DateFormat iso8601Format;
    ...
    private Date deserializeToDate(final JsonElement json) {
        synchronized(enUsFormat) {
            try {
                ...
                catch(final ParseException localParseException1) {
                    try {
    ...
    public String serializeToString(final Date date) {
        synchronized(enUsFormat) {
            final String result = iso8601Format.format(date);
