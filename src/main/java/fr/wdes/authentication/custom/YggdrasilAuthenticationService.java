...
import fr.wdes.authentication.BaseAuthenticationService;
...
public class YggdrasilAuthenticationService extends BaseAuthenticationService {
    ...
    */
	protected static final String BASE_URL = LauncherConstants.URL_AUTHENTIFICATION_SYSTEM+"/";
	protected static final URL ROUTE_AUTHENTICATE = Util.constantURL(LauncherConstants.URL_AUTHENTIFICATION_SYSTEM+"/authenticate.php");
	protected static final URL ROUTE_REFRESH = Util.constantURL(LauncherConstants.URL_AUTHENTIFICATION_SYSTEM+"/refresh.php");
	protected static final URL ROUTE_VALIDATE = Util.constantURL(LauncherConstants.URL_AUTHENTIFICATION_SYSTEM+"/validate.php");
	protected static final URL ROUTE_INVALIDATE = Util.constantURL(LauncherConstants.URL_AUTHENTIFICATION_SYSTEM+"/invalidate.php");
	protected static final URL ROUTE_SIGNOUT = Util.constantURL(LauncherConstants.URL_AUTHENTIFICATION_SYSTEM+"/signout.php");
    protected static final String STORAGE_KEY_ACCESS_TOKEN = "accessToken";
    protected final Gson gson = new Gson();
    protected final Agent agent = Agent.MINECRAFT;
    ...
    protected void logInWithPassword() throws AuthenticationException {
        ...
        logger.info("Connection par nom d'utilisateur & mot de passe");
        Launcher.getInstance().getLauncherPanel().getProgressBar().setWaiting(true);
        Launcher.getInstance().getLauncherPanel().getProgressBar().setText("Connexion en cours ...");
        ...
        if(!response.getClientToken().equals(getClientToken()))
            throw new AuthenticationException("Le serveur demande a changer le token client mais comment faire ??");
    ...
    protected void logInWithToken() throws AuthenticationException {
        ...
        Launcher.getInstance().getLauncherPanel().getProgressBar().setWaiting(true);
        Launcher.getInstance().getLauncherPanel().getProgressBar().setText("Connexion avec token d'accès en cours ...");
    ...
    protected <T extends Response> Response makeRequest(final URL url, final Object input, final Class<T> classOfT) throws AuthenticationException {
        try {
            jsonResult = Util.performPost(url, gson.toJson(input), Launcher.getInstance().getProxy(), "application/json", true);
    ...
    public void selectGameProfile(final GameProfile profile) throws AuthenticationException {
        ...
        if(profile == null || !ArrayUtils.contains(profiles, profile))
            throw new IllegalArgumentException("Profil invalide : '" + profile + "'");
