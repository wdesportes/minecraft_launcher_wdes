  ...
  extends Downloadable
{
    ...
    throws IOException
  {
    ...
    if (status / 100 == 2)
    {
      ...
      if (hash.equalsIgnoreCase(this.asset.getHash())) {
        return "Téléchargé et hash vérifié";
      }
      FileUtils.deleteQuietly(localAsset);
    ...
    throws IOException
  {
    ...
    FileUtils.deleteQuietly(localAsset);
    throw new RuntimeException("Had local compressed fond but unpacked hash did not match (expected " + this.asset.getHash() + " but had " + hash + ")");
  }
