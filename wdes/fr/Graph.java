...
private BufferedImage fond()
{
 List<File> images = new ArrayList<File>();
  File backgroundDir = new File(new File(Util.getWorkingDirectory(Parametres.getparam("APPDATA")), "fonds"), heure());
 ...
 try
 {
   ...
   catch (Exception io)
   {
     if (images.size() > 0) {
      ...
      stream = Ressources.getResourceAsStream("/wdes/fr/ressources/4.jpg");
    }
...
class Box extends JTextField implements FocusListener
{
   private static final long serialVersionUID = 1L;
   protected final JLabel label;
   
  public Box(Connection connection, String label)
   {
     this.label = new JLabel(label);
     addFocusListener(this);
     connection.getRootPane().add(this.label);
